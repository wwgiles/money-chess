...
-- Add new columns for game configuration if they don't exist
ALTER TABLE game_sessions ADD COLUMN IF NOT EXISTS game_name TEXT DEFAULT 'Untitled Game';
ALTER TABLE game_sessions ADD COLUMN IF NOT EXISTS is_private BOOLEAN DEFAULT FALSE;
ALTER TABLE game_sessions ADD COLUMN IF NOT EXISTS password TEXT;
ALTER TABLE game_sessions ADD COLUMN IF NOT EXISTS fog_of_war_enabled BOOLEAN DEFAULT FALSE;
ALTER TABLE game_sessions ADD COLUMN IF NOT EXISTS move_time_limit INTEGER DEFAULT 300; -- in seconds, default 5 minutes
